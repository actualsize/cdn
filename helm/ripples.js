...
export class WaterCanvas {
	constructor(width, height, documentElement, waterModel, props) {
...
/**
 * A export class to mimic rain on the given waterModel with raindrop2dArray's as raindrops.
 */
	...
	canvasHolder.addEventListener(
		...
		function (e) {
			...
			waterModel.touchWater(
				...
				y,
				1.5,
				mouseDown ? window.finger : window.pixel
			);
		},
		false
	);
...
const waterCanvas = new WaterCanvas(width, height, "waterHolder", waterModel, {
	backgroundImageUrl: "../images/logo.png",
	lightRefraction: 12.0,
	lightReflection: 0.0125,
	maxFps: 20
});

window.pixel = create2DArray(createRadialCanvas(8, 8));
